...
def test_1():
    for user_id in range(1000000000, 1000000100, 10):
        ...
        for favorit_id in range(user_id - 10, user_id, 2):
            add_favorites(user_id, favorit_id)
    input('push inter:')
    for user_id in range(1000000000, 1000000100, 10):
        print(user_id, is_user(user_id))
        print(not (is_user(user_id) is None))
        for favorit_id in range(user_id - 10, user_id, 2):
            print('       ', favorit_id, not (is_favorites(favorit_id) is None))
            print('       ', not (is_user_favorites(user_id, favorit_id) is None))
    input('push inter:')
    for user_id in range(1000000000, 1000000100, 10):
        assert not (is_user(user_id) is None)
        for favorit_id in range(user_id - 10, user_id, 2):
            assert not (is_favorites(favorit_id) is None)
            assert not (is_user_favorites(user_id, favorit_id) is None)
